...
final class CloudKitSyncEngine: @unchecked Sendable {
    ...
    // Lazy: constructing a CKContainer touches CloudKit.framework immediately, which
    // traps on builds signed without the iCloud entitlement (e.g. free-account sideloads).
    // Deferring construction until sync is actually enabled avoids crashing on every launch.
    private lazy var container: CKContainer = CKContainer(identifier: AppIdentifiers.iCloudContainer)
    private lazy var database: CKDatabase = container.privateCloudDatabase
    private lazy var zoneManager: CloudKitZoneManager = CloudKitZoneManager(database: database)
    ...
    init(
        ...
        // container/database/zoneManager are lazy now — not touched here, only when sync runs.
